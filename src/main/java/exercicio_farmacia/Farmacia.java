...
public class Farmacia {
	...
	public boolean vender(Cliente c, Produto p, int qtd) {
		if (p instanceof Medicamentos) {
			return false;
		}
		return p.venda(c, qtd);
	}

	public boolean venderMedicamentoReceita(Cliente c, Produto p, int qtd, String nomeMedico) {
		if (p instanceof Medicamentos) {
			((Medicamentos) p).defineNomeMedico(nomeMedico);
		}
		return p.venda(c, qtd);
	}
